import {addChain} from "../../utils/web3";

function AddChainToWallet({chainId, chainName, chainDisplayName,  rpcUrls, nameOfNativeCurrency, blockExplorerUrls}) {

    return (
        <>
            <button onClick={() => addChain(chainId, chainName, rpcUrls, nameOfNativeCurrency, blockExplorerUrls)}
                    className={`bg-gradient-to-r from-purple-600 to-pink-500 focus:outline-none text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2 mb-2 dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-900`}>
                <p className="relative z-20">Add {chainDisplayName} <br/>to Metamask</p>
            </button>
        </>
    )
}

export default AddChainToWallet